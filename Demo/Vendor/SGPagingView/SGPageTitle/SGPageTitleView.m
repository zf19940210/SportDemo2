...
/** 根据下标值添加 badge */
- (void)addBadgeForIndex:(NSInteger)index withIndex:(int)tag{
    ...
    CGFloat badgeX = 0.4 * (btn.SG_width - btnTextWidth) + btnTextWidth + self.configure.badgeOff.x;
    CGFloat badgeY = 0.73 * (btn.SG_height - btnTextHeight) + self.configure.badgeOff.y - self.configure.badgeSize;
    ...
    badge.text = [NSString stringWithFormat:@"%d",tag];
...
/** 根据下标值添加 badge */
/*
- (void)addBadgeForIndex:(NSInteger)index withIndex:(int)index{
    UIButton *btn = self.btnMArr[index];
    //UIView *badge = [[UIView alloc] init];
    UILabel *badge = [[UILabel alloc]init];
    CGFloat btnTextWidth = [self P_sizeWithString:btn.currentTitle font:self.configure.titleFont].width;
    ...
    CGFloat badgeWidth = self.configure.badgeSize;
    CGFloat badgeHeight = badgeWidth;
    badge.text = [NSString stringWithFormat:@"%d",index];
    badge.font = [UIFont systemFontOfSize:10];
    badge.textAlignment = NSTextAlignmentCenter;
    badge.textColor = [UIColor whiteColor];
    badge.frame = CGRectMake(badgeX, badgeY, badgeWidth, badgeHeight);
